...
function SimpleExtractTwigTemplate()
  if vim.bo.filetype ~= 'twig' then
    print 'Not a Twig file!'
    return
  end
  ...
  -- If only one line is selected, convert it to a table
  if type(component_lines) == 'string' then
    component_lines = { component_lines }
  end
...
function SimpleExtractTwigTemplateWithProps()
  if vim.bo.filetype ~= 'twig' then
    print 'Not a Twig file!'
    return
  end
...
vim.api.nvim_set_keymap('x', '<Leader>et', ':lua SimpleExtractTwigTemplate()<CR>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('x', '<Leader>ep', ':lua SimpleExtractTwigTemplateWithProps()<CR>', { noremap = true, silent = true })
